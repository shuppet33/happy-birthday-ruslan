...
def test1(source_code: str) -> Dict[str, str]:
    ...
    for i, test_tree in enumerate(trees):
        try:
            ...
            return {"res": '', "error": f"Test failed with error: {e}"}

    print(test_result)
    if test_result:
        return {"res": str(all(test_result)), "error": ''}
    else:
        return {"res": "False", "error": ''}


def test2(source_code: str) -> Dict[str, str]:
